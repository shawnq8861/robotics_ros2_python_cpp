...
class MotorStart : public rclcpp::Node
{
public:
    MotorStart()
    : Node("motor_start"), count_(0)
    {
    }
...
int main(int argc, char * argv[])
{
    ...
    // create the node
    //
    auto motor_encoder_test_node = std::make_shared<MotorStart>();
    //
    // use rt extensions
    //
    int rc = -1;
    ...
    RCLCPP_INFO_STREAM(motor_encoder_test_node->get_logger(), "motor start... ");
    //
    // try to open a port
    //
